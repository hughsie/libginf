 ...
 * @stability: Stable
 *
 * These functions are used internally to libginf, and some may be
...
/**
 * ginf_keyfile_error_quark:
 *
 * Return value: An error quark.
 *
 * Since: 0.3.7
 **/
G_DEFINE_QUARK (ginf-common-error-quark, ginf_keyfile_error)
...
static gchar *
ginf_keyfile_make_case_insensitive (GInfKeyfileHelper *helper, const gchar *text)
{
	if (helper->flags & GINF_KEYFILE_LOAD_FLAG_CASE_INSENSITIVE)
		return g_ascii_strdown (text, -1);
	return g_strdup (text);
}

static gboolean
ginf_keyfile_string_isdigits (const gchar *str)
{
	guint i;
	for (i = 0; str[i] != '\0'; i++) {
...
static gchar *
ginf_keyfile_replace_variable (GInfKeyfileHelper *helper, const gchar *line, GError **error)
{
	GString *new;
	const gchar *tmp;
	guint i;
	...
	for (i = 0; split[i] != NULL; i++) {
		...
		/* a Dirid */
		if (ginf_keyfile_string_isdigits (split[i])) {
			g_string_append (new, "/tmp");
			continue;
		}

		/* replace or ignore things not (yet) in the dictionary */
		lower = ginf_keyfile_make_case_insensitive (helper, split[i]);
		tmp = g_hash_table_lookup (helper->dict, lower);
		if (tmp == NULL) {
			if (helper->flags & GINF_KEYFILE_LOAD_FLAG_STRICT) {
				g_set_error (error,
					     GINF_ERROR,
					     GINF_ERROR_FAILED,
...
static GHashTable *
ginf_keyfile_get_dict (GInfKeyfileHelper *helper, GError **error)
{
	...
	lower = ginf_keyfile_make_case_insensitive (helper, "Strings");
...
static gboolean
ginf_keyfile_replace_variables (GInfKeyfileHelper *helper, GError **error)
{
	guint i;
	guint j;
	...
	for (i = 0; groups[i] != NULL; i++) {
		...
		for (j = 0; keys[j] != NULL; j++) {
			...
			data_new = ginf_keyfile_replace_variable (helper,
							    data_old,
							    error);
			if (data_new == NULL)
				return FALSE;
...
static void
ginf_keyfile_set_last_key (GInfKeyfileHelper *helper, const gchar *key)
{
	/* same value */
...
static void
ginf_keyfile_set_comment (GInfKeyfileHelper *helper, const gchar *comment)
{
	/* same value */
...
static gboolean
ginf_keyfile_strcheckchars (const gchar *str, const gchar *chars)
{
	guint i;
	guint j;
	for (i = 0; str[i] != '\0'; i++) {
...
static gboolean
ginf_keyfile_strip_value (GInfKeyfileHelper *helper, gchar *value, GError **error)
{
	gchar *comment;
	gchar *last_quote;
	...
	if (value[0] != '"') {
		...
		if (comment != NULL) {
			*comment = '\0';
			if (helper->comment == NULL)
				ginf_keyfile_set_comment (helper, comment + 1);
			g_strchomp (value + 1);
		}
		return TRUE;
	}
...
static gboolean
ginf_keyfile_set_group (GInfKeyfileHelper *helper, const gchar *group, GError **error)
{
	/* same value */
	...
	/* save new value */
	g_free (helper->group);
	helper->group = ginf_keyfile_make_case_insensitive (helper, group);

	/* section names can contain double quotes */
	if (!ginf_keyfile_strip_value (helper, helper->group, error))
		return FALSE;
	...
	if (helper->flags & GINF_KEYFILE_LOAD_FLAG_STRICT) {
		...
		if (ginf_keyfile_strcheckchars (helper->group, "\t\n\r;\"")) {
			g_set_error (error,
				     GINF_ERROR,
				     GINF_ERROR_FAILED,
	...
	/* no longer valid */
	ginf_keyfile_set_last_key (helper, NULL);
	...
	if (helper->comment != NULL) {
		g_key_file_set_comment (helper->keyfile,
					...
					helper->comment,
					NULL);
		ginf_keyfile_set_comment (helper, NULL);
	}
	return TRUE;
}

static void
ginf_keyfile_set_key (GInfKeyfileHelper *helper, const gchar *key, const gchar *value)
{
	helper->last_line_continuation_ignore = FALSE;
	...
	ginf_keyfile_set_last_key (helper, key);
}

static void
ginf_keyfile_convert_key (gchar *key)
{
	guint i;
	for (i = 0; key[i] != '\0'; i++) {
...
static gboolean
ginf_keyfile_repair_utf8 (GInfKeyfileHelper *helper, gchar *line, GError **error)
{
	guint i;
	guint8 val;
...
static gboolean
ginf_keyfile_parse_line (GInfKeyfileHelper *helper, gchar *line, GError **error)
{
	gchar *kvsplit;
	gchar *comment;
	gchar *tmp;
	...
	/* line too long */
	if ((helper->flags & GINF_KEYFILE_LOAD_FLAG_STRICT) > 0 &&
	    strlen (line) > 4096) {
		ret = FALSE;
		g_set_error (error,
			     GINF_ERROR,
	...
	if (!ginf_keyfile_repair_utf8 (helper, line, error)) {
		ret = FALSE;
		goto out;
	}
	...
	/* remove comments */
	if (line[0] == ';') {
		ginf_keyfile_set_comment (helper, line + 1);
		goto out;
	}
	...
	if (line[0] == '[') {
		...
		*tmp = '\0';
		ret = ginf_keyfile_set_group (helper, line + 1, error);
		goto out;
	}
	...
	if ((tmp == NULL && kvsplit != NULL) ||
		...
		*kvsplit = '\0';
		key = ginf_keyfile_make_case_insensitive (helper, line);
		g_strchomp (key);

		/* remove invalid chars as GKeyFile is more strict than .ini */
		ginf_keyfile_convert_key (key);
		...
		if (key[0] == '%') {
			...
			key_tmp = ginf_keyfile_replace_variable (helper, key, error);
			if (key_tmp == NULL)
				return FALSE;
			if (key_tmp[0] == '%') {
			...
			key_new = ginf_keyfile_make_case_insensitive (helper, key_tmp);
			ginf_keyfile_convert_key (key_new);
		} else {
		...
		g_strchug (kvsplit + 1);
		if (!ginf_keyfile_strip_value (helper, kvsplit + 1, error)) {
			ret = FALSE;
			goto out;
		}

		/* add to keyfile */
		ginf_keyfile_set_key (helper, key_new, kvsplit + 1);
		goto out;
	}
	...
	if (helper->last_line_continuation) {
		...
		/* remove leading and trailing quote */
		if (!ginf_keyfile_strip_value (helper, line, error)) {
			ret = FALSE;
			goto out;
		}
		...
		ginf_keyfile_set_key (helper, helper->last_key, new);
		goto out;
	}
	...
	if (g_strcmp0 (helper->group, "Firmware_AddReg") == 0 &&
		...
		if (str->len > 0) {
			...
			if (!ginf_keyfile_strip_value (helper, reg_split[i], error)) {
				ret = FALSE;
				goto out;
			}
			if (helper->dict == NULL) {
			...
			key_tmp = ginf_keyfile_replace_variable (helper, str->str, error);
			if (key_tmp == NULL)
				return FALSE;
			ginf_keyfile_set_key (helper, key_tmp, reg_split[i]);
			goto out;
		}
	}
	...
	ginf_keyfile_set_key (helper, key, line);
out:
	helper->last_line_continuation = continuation;
	return ret;
}

static GInfKeyfileHelper *
ginf_keyfile_helper_new (void)
{
	GInfKeyfileHelper *helper;
	helper = g_new0 (GInfKeyfileHelper, 1);
	return helper;
}

static void
ginf_keyfile_helper_free (GInfKeyfileHelper *helper)
{
	if (helper->dict != NULL)
...
/**
 * ginf_keyfile_load_data:
 * @keyfile: a #GKeyFile
 * @data: the .inf file date to parse
 * @flags: #GInfKeyfileLoadFlags, e.g. %GINF_KEYFILE_LOAD_FLAG_NONE
 ...
 */
gboolean
ginf_keyfile_load_data (GKeyFile *keyfile,
		  const gchar *data,
		  GInfKeyfileLoadFlags flags,
		  GError **error)
{
	...
	/* initialize helper */
	helper = ginf_keyfile_helper_new ();
	helper->flags = flags;
	...
	for (i = 0; lines[i] != NULL; i++) {
		if (!ginf_keyfile_parse_line (helper, lines[i], error)) {
	...
	helper->dict = ginf_keyfile_get_dict (helper, error);
	if (helper->dict == NULL) {
		ret = FALSE;
		goto out;
	}
	...
	if (helper->require_2nd_pass) {
		...
		for (i = 0; lines2[i] != NULL; i++) {
			if (!ginf_keyfile_parse_line (helper, lines2[i], error)) {
				g_prefix_error (error,
						"Failed to parse line %u: ",
						i + 1);
				ret = FALSE;
	...
	/* replace key values */
	if (!ginf_keyfile_replace_variables (helper, error)) {
		ret = FALSE;
		goto out;
	}
out:
	ginf_keyfile_helper_free (helper);
	return ret;
}
 ...
 */
gboolean
ginf_keyfile_load_file (GKeyFile *keyfile,
		  ...
		  GError **error)
{
	...
	GInfKeyfileBOM boms[] = { { "\x00\x00\xfe\xff",	"UTF-32BE",	4 },
	...
	return ginf_keyfile_load_data (keyfile, data_no_bom, flags, error);
}

/**
 * ginf_keyfile_get_driver_version:
 * @keyfile: a #GKeyFile
 ...
 */
gchar *
ginf_keyfile_get_driver_version (GKeyFile *keyfile, guint64 *timestamp, GError **error)
{
